...
function EasyMail.OnEvent(self, event, ...)
	...
	if ( event == "PLAYER_LOGIN" ) then
		...
		if (not EasyMail_SavedVars.AttReport) then
			EasyMail_SavedVars.AttReport = "Y";
		end
		...
		-- Hook the take attachment function to output to chat frame
		hooksecurefunc("TakeInboxItem", EasyMail.PrintAttachment);
...
---------------------------------------------------
-- EasyMail.PrintAttachment
-- Output attachment received from mail
---------------------------------------------------
function EasyMail.PrintAttachment(mailID, attachmentIndex)
	if (EasyMail_SavedVars.AttReport ~= "Y") then
		return;
	end
	
	local name, _, count = GetInboxItem(mailID, attachmentIndex);
	...
	EasyMail.Print("|cffffb07f"..format(EASYMAIL_RECEIVEATT, link).."|cffffb07f "
		..format(EASYMAIL_FROM, OpenMailSender:GetText()));
end

...
--[[ Author: Taffu  RevDate: 10/06/2012  Version: 1.1.0 ]]
...
local guildName, guildMotto, guildRank, guildLevel, guildList
...
local function SetupToolTip()
	...
	tool:SetScript("OnEnter", function(this)
		...
		SLT:AddHeader("SLDT_Guild", string.format("%s (%s)", guildName or L["No Guild"], guildLevel or ""), guildMotto)
...
function SLDT.Guild:Refresh()
	if ( db.enabled or SLDataText.db.profile.configMode ) then
		...
		if ( IsInGuild() ) then
			...
			guildMotto, guildLevel = GetGuildRosterMOTD(), GetGuildLevel()
			...
			for i = 0, select(1, GetNumGuildMembers()) do
				...
				if ( online ) then
					...
					table.insert(guildList, { name, rank, level, classFileName, zone, status, isMobile, note, officernote })
				end
			end

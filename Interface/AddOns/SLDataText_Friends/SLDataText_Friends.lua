...
--[[ Author: Taffu  RevDate: 09/02/2012  Version: 1.0.5 ]]
	...
	tool:SetScript("OnEnter", function(this)
		...
		if ( BNetList and select("#", BNetList) >= 1 ) then
			for k, v in pairs(BNetList) do
				...
				local lineL = string.format("|cffecd672%s|r | |cffecd672%s|r (%s)", BNname, battleTag, toonname)
	...
	if ( db.enabled or SLDataText.db.profile.configMode ) then
		...
		for j = 1, BNGetNumFriends() do
			...
			if ( online ) then
				...
				friendList = friendList or {}
				local status = ""
				if ( isafk ) then status = L["(AFK)"] end; if ( isdnd ) then status = L["(DND)"] end
